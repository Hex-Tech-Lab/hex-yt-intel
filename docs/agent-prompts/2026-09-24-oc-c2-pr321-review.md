# Agent Dispatch Prompt — PR #321 round 2 — external review findings
...
**PR #321** (`fix/observability-blind-spots`, head `cb5d1aa`) received an external review. Raw findings — verify each before acting; fix only confirmed ones; report rejections with evidence:

P0. Failing checks: CodeFactor (https://www.codefactor.io/repository/github/hex-tech-lab/hex-yt-intel/pull/321), DeepSource JavaScript (web), and Netlify deploy-preview / header rules / redirect rules / pages changed (https://app.netlify.com/projects/hex-yt-intel/deploys/6ab45c72f30f9f000839c15c). Get exact findings; fix actionable ones. For Netlify, determine whether it fails on main too (pre-existing) before changing anything.
P1. `web/lib/admin-logs/fetchers.ts`: the Observability query filters only on `$metadata.error` exists, so an event with `$workers.outcome = "exceededCpu"` but no `$metadata.error` is dropped. Return outcome events too (verified OR filter, or drop the server filter and filter in code). Test: outcome-only event with no metadata.error reaches the snapshot. Live reference (2026-09-23 20:39 UTC): request events carry `$workers.outcome: "exceededCpu"`; a separate event carries message "Worker exceeded CPU time limit." — check with the real API using `CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID` from `web/.env.local` (read-only).
P1. The Observability fetch has no timeout: add an abort-signal bound (value from Settings Registry or an existing config constant per the no-hardcoded-tunables directive); on timeout keep GraphQL data, 200 + warning; test that it settles.
P1. `limit: 100` truncates silently: define the completeness contract — paginate within the window, or expose `truncated` + warning. Test >100 events.
P1/P2. Malformed/unknown response shape currently becomes an empty list silently: validate the shape at runtime (Zod), distinguish valid empty `result.events.events: []` from malformed; fail soft with a warning; test the wrong shape.
P2. External `error`/`message` fields interpolated raw into newline-delimited log text → newline/control-char injection can forge log lines. Sanitize; test newline-bearing values.
P2. Tests: also pin µs→ms CPU conversion, aggregate `errors` count, request payload filters.
P2. `totalEntries` now counts GraphQL + Observability lines while `invocations` holds only aggregates: trace every consumer of the snapshot (admin logs UI, `scripts/poll-logs-snapshot.sh` users) and define the semantics explicitly (add fields if needed) with a contract test.
P2. A comment still documents `result.events[]`; the real shape is `result.events.events[]` — fix the comment.
...
Work through the findings in priority order in branch `fix/observability-blind-spots`. Also: the PR currently contains your earlier `[IN_PROGRESS]` ledger line — change it to `[DONE]` with the real commits/verification. The dispatch prompt file in `docs/agent-prompts/` stays (project convention: every dispatch prompt is archived there); do not delete it. Gates: web tsc, affected vitest suites (and full web vitest before finishing), worker typecheck (`tsconfig.typecheck.json`) + build if worker touched, qa-intel diff. Report per finding: fixed / rejected (evidence) / needs user decision.
