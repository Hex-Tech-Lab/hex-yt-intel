# Agent Dispatch Prompt — PR #322 round 2 — external review findings
...
**PR #322** (`feat/stance-dual-persistence-wordcloud`, head `8c159c92`, authored by AGY, re-verified by CC for tests/tsc only). External review raw findings — verify each, fix confirmed ones, report rejections with evidence:

P1. `web/app/api/analyses/[id]/relations/route.ts` and `scripts/backfill-stance-relations.ts` do a full `analysis_payload` read-modify-write from a stale snapshot → concurrent payload writes (persist finalize, highlights, remediation) can be lost. Update ONLY `analysis_payload.stance_relations` atomically (database-side `jsonb_set` via an RPC/migration, or an equivalent atomic path). One shared helper (port + adapter) used by both route and script. Concurrency regression test proving an unrelated key survives.
P1. Valid empty results are never persisted (`if (insights.length > 0)`) → recompute every Redis expiry, paying LLM tokens forever. Persist successful zero-insight results; do NOT cache genuine failures (model/network exhaustion). Needs an explicit success marker.
P1. Supabase persistence errors are logged and ignored while the request succeeds; zero-row updates unnoticed. Bounded retry, verify affected row, Sentry capture; decide & document the contract (fail vs durable repair).
P1. Backfill falls back from SUPABASE_SERVICE_ROLE_KEY to NEXT_PUBLIC_SUPABASE_ANON_KEY → can silently scan 0 rows under RLS. Require the service key; fail fast.
P1. Failing checks on head: Lint, Pipeline Status, CodeFactor, DeepSource web + worker. Reproduce exact findings and fix.
P2. Route selects `analysis_payload` before the Redis lookup, so cache hits transfer the full JSONB. Fetch only what the Redis key needs first; read the stance payload only on miss. Test.
P2. Backfill Redis SET puts JSON in the URL path and ignores non-2xx. Use POST/body (or the existing Upstash adapter in the repo), check `response.ok`, report Redis vs Supabase separately.
P2. Backfill hard-caps at 100 rows with no pagination → silent partial coverage. Paginate or fail loudly when rows remain; report scanned/skipped/failed/remaining.
P2. `web/lib/intelligence/relations-engine.ts` new recovery blindly appends `]}` after the last `}` — can accept a truncated prefix and persist it permanently. Replace with bracket-aware repair (the repo already depends on `jsonrepair`), validate the result, mark partial or fall through to the next model; tests for the truncation shapes.
P2. `kg-relations.contract.test.ts` ADR 031 assertions build an in-memory object and never exercise the route → add mocked route-level tests: Redis hit, Redis miss + Supabase hit, stale content hash, non-empty persist, empty persist, Redis failure, Supabase failure, unrelated-key preservation.
Do NOT re-run the backfill against production.
