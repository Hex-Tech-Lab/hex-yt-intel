...
gh api --paginate --slurp 'repos/{owner}/{repo}/code-scanning/alerts?state=open' | jq 'add | length'
gh api --paginate --slurp 'repos/{owner}/{repo}/dependabot/alerts?state=open' | jq 'add | length'
```
