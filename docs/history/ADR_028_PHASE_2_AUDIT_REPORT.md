...
**1.1 Ingestion Trace**
*   **[web/lib/usecases/ExtractHighlightsUseCase.ts:203-219](web/lib/usecases/ExtractHighlightsUseCase.ts#L203-L219)**: Modifies the finalization step of the extraction pipeline. Once semantic highlights are built and saved, it triggers the secondary SimHash pipeline if valid transcript segments exist.
*   **[web/lib/utils/simhash.ts:32-45](web/lib/utils/simhash.ts#L32-L45)**: Translates 30-second temporal windows of normalized transcript text into robust `64-bit BigInt` fingerprints.
*   **[web/lib/ports/TemporalKnowledgePort.ts:18](web/lib/ports/TemporalKnowledgePort.ts#L18)**: Abstraction boundary for persisting `TemporalSubgraphNode[]` matrices.
*   **[web/lib/adapters/SupabaseTemporalGraphAdapter.ts:16-30](web/lib/adapters/SupabaseTemporalGraphAdapter.ts#L16-L30)**: Adapts the anchor array into structured multi-row `.insert()` queries targeting `analysis_simhash_anchors`.
*   **[web/app/api/webhooks/highlights/route.ts:52-57](web/app/api/webhooks/highlights/route.ts#L52-L57)**: DI container wiring `SupabaseTemporalGraphAdapter` into `ExtractHighlightsUseCase`.

**1.2 Consumption Trace (Grounding)**
*   **[web/app/api/chat/conversations/[id]/messages/route.ts:104-110](web/app/api/chat/conversations/[id]/messages/route.ts#L104-L110)**: Injects the graph adapter into the chat handler boundary.
*   **[web/lib/usecases/ProcessChatMessageUseCase.ts:468-477](web/lib/usecases/ProcessChatMessageUseCase.ts#L468-L477)**: Detects expired (>72h) transcript context (`!groundingData.transcript`). Invokes fallback temporal graph bridging (`temporalGraph.queryTemporalSubgraph`), serializing nodes efficiently back into the LLM synthesis window.

## 2. Dead-Code Attestation

**Status: VERIFIED MOCK UTILIZATION**
...
A dedicated mock use-case test suite covering integration logic and temporal query edge cases was authored and verified locally.
