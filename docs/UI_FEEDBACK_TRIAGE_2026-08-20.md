...
1. **Dimension 11 (Commercial Yield) table — last column fully broken.** `11.6 Monetization Risk & Sustainability Assessment` table: the "Mitigation Path" column content is rendering as a single vertical stack of individual characters (one letter per line), not wrapped text. Screenshot shows literal `S/p/e/c/i/a/l/i/z/e/...` stacked vertically. Needs the table/cell CSS found and the column given real `white-space`/`overflow-wrap` handling instead of whatever is currently collapsing it character-by-character.
...
11. **`EntityMentionTimeline`'s slide-down UI does not mount on entity click in current production, despite `setSeekTo` firing.** Found while investigating item 8's redesign (2026-08-20). Quick diagnostic pass only, per dispatch scope -- NOT fixed here, needs its own investigation.
    - Mount site: `web/components/containers/DashboardContainer.tsx` renders `<EntityMentionTimeline>` only when `timelineEntityData` (a `useMemo`, lines ~302-320) is non-null, which itself requires `getRankedMentionsForEntity(...).mentions.length > 1` (the memo explicitly returns `null` for `<= 1`).
    - `getRankedMentionsForEntity` (`web/lib/utils/entity-time-seek.ts:407`) returns an empty `mentions` array immediately whenever `findAllEntityMentions(...)` finds zero matches (line ~417-419) -- and per this project's own tracked issue, that mention-identification/ranking pipeline is the same one already flagged elsewhere as producing wrong/repeated seek targets (see this dispatch's CRITICAL CORRECTION in section 1 of `docs/agent-prompts/2026-08-20-cc-highlights-reel-redesign.md`). The leading hypothesis is that `findAllEntityMentions` is now typically resolving 0 or 1 real mention per entity (rather than the multiple it used to), which would explain both symptoms together: wrong/repeated seek targets when the (single, mis-ranked) match still fires a seek via whatever click handler calls `setSeekTo` directly, AND the timeline component never mounting since its own `<= 1` gate is never cleared.
    - Not confirmed live (would need a real analysis + entity click + console inspection of `matches.length`) -- flagged as the most likely root cause for whoever picks this up next, not a verified fix.
