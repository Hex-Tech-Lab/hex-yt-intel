...
| 8 | 16 | `req.tier` read but never sent — every user's SSE meta frame reports `tier: "free"` | `worker/src/chat-stream.ts:328`, `web/lib/usecases/ProcessChatMessageUseCase.ts` | open | Found as a pre-existing `tsc` error while verifying an unrelated change (`Property 'tier' does not exist on type 'ChatStreamRequest'`) — investigated rather than dismissed as noise. `ChatStreamRequest` has no `tier` field and `ProcessChatMessageUseCase` confirms it never sends one; `req.tier \|\| "free"` is always the fallback. Cosmetic only — actual model selection correctly uses `cascade`/`models`, which *are* wired — but a paying user's client-side "meta" event shows the wrong tier label. Fix: either thread `tier` through the use case's worker payload and the `ChatStreamRequest` interface, or remove the dead read if the field is genuinely not meant to reach the worker. |
