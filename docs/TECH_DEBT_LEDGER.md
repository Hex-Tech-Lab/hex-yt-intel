...
| 12 | 13 | `ERR_NO_TRANSCRIPT` conflates "genuinely no captions" with "extraction pipeline exhausted (Decodo + YouTube Native + page-HTML all failed)" | `worker/src/services/TranscriptExtractor.ts` (`fetchWithTertiary`), `worker/src/routes/analysis.ts:834`, `web/hooks/useSSEStream.ts:398-400` | open | **Corrected from prior triage** — `TranscriptExtractor.ts` is actually well-instrumented (my earlier "0 logging calls" was a grep-pattern miss: the file uses `console.warn`/`captureException`, not the literal strings I searched for; read line-by-line this time). Real finding one layer down: when all 3 fallback tiers fail, `fetchWithTertiary` returns a placeholder string (`"[Transcript unavailable...]"`), which `analysis.ts:834` correctly detects and converts to `ERR_NO_TRANSCRIPT` — not corrupted, not silent, each tier's failure IS captured to Sentry individually. But the client (`useSSEStream.ts:398-400`) shows the **identical** "this video has no captions, try another video" message for both "video genuinely has none" and "our extraction pipeline had an outage" — same misdiagnosis pattern as item 1, one layer removed, and actively bad advice in the outage case (retrying or trying a different video won't help; the fix is to retry *this* video later). Per-tier Sentry breadcrumbs already exist for RCA by videoId+timestamp — the gap is only in the user-facing message, not telemetry. Needs a product call: is a distinct "transcript extraction is temporarily unavailable, try again shortly" message worth the extra error-code plumbing, or is "can't analyze this video" an acceptable unified outcome regardless of cause? Not implementing without that call. |
...
- `worker/src/services/TranscriptExtractor.ts` — read line-by-line (not just grep-triaged): well-instrumented, no fix needed at that layer. Real finding moved one layer downstream to the `ERR_NO_TRANSCRIPT` message-conflation issue (item 13, rescored 12).
- `MetadataScraper.ts` (item 14) and `CommentClassifier.ts` (item 15) are still grep-count triage only — do the line-by-line read before writing any fix for either; don't trust the ratio alone after being burned by it on `TranscriptExtractor.ts`.
