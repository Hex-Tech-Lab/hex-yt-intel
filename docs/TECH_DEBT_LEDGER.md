...
| 12 | 19 | Dimension-remediation never regenerates digest/highlights after backfilling a missing core dimension | `web/lib/services/dimension-remediation.ts`, `web/app/api/analyses/digest/route.ts`, `web/lib/usecases/GenerateExecutiveDigestUseCase.ts` | **deferred** | Found 2026-08-21 investigating the highlights/chat/digest consistency work. Remediation only ever backfills missing CORE dimensions (1-11) — confirmed via direct grep, zero references to `highlights`/`digest`/`executive` in `dimension-remediation.ts`/`aux-remediation.ts`. If a takeaway or highlight later gets marked "not grounded" (new reconciliation step, in progress) because the source dimension was missing at extraction time, there is no self-healing path — remediation fixing that dimension afterward never re-triggers digest/highlights regeneration. Real fix requires two things, not one: (1) relax `GenerateExecutiveDigestUseCase`'s current strict "generate once, ever" idempotency to allow a deliberate one-time regeneration specifically triggered by a completed remediation event, and (2) a new integration point hooking dimension-remediation's completion into that trigger. Explicit user decision (2026-08-21): real, worth doing, but a separate piece of work — not bundled into the current consistency-fix dispatch. Priority: medium — genuine data-completeness gap affecting any analysis that goes through remediation, but not blocking any current user-facing feature.
| 6 | 20 | Live `cascade.chat` registry value (not just the code fallback) is stale on Gemini's escalation tier, untouched since seeding | `setting_values` row `cascade.chat` (Supabase), `web/lib/config/cascade.ts` `CHAT_CASCADE_FALLBACK` | open | Found + confirmed live 2026-08-21. This is **registry-driven and already admin-UI-editable** (`/admin/settings` → "LLM Cascades" category, confirmed via `AdminSettingsClient.tsx`'s real `cascade: 'LLM Cascades'` category) — NOT a hardcoded-value gap as first framed. Direct query of the live `setting_values` table confirmed the actual production value is `google/gemini-3.5-flash-lite`, `updated_at: 2026-07-25T17:17:35Z` — byte-identical to the code fallback, never edited since it was seeded. `REASONING_CASCADE_PRO_FALLBACK` in the SAME `cascade.ts` file already uses `google/gemini-3.6-flash`, confirming a newer model was already adopted elsewhere and just never carried over here. Fix is a live admin-settings edit (both the DB row and the code fallback constant, so they stay in sync) — not a code refactor. Deliberately not done in this pass (out of scope for the highlights/digest/chat work that surfaced it) — needs a real check of current Gemini Flash-tier pricing/capability before picking a replacement, not a blind version bump. See item #21 below for the follow-up feature (OpenRouter-API-backed slug validation) that would prevent this class of drift going forward.
...
| 8 | 21 | Feature idea: validate cascade model slugs against OpenRouter's live `/models` catalog | `web/lib/config/cascade.ts`, `web/app/admin/settings/AdminSettingsClient.tsx` (admin UI edit path) | open | Logged 2026-08-21, explicit user request, priority defined: score 8 (severity 2 — degraded behavior, a bad/stale slug fails silently at completion-call time rather than at edit time; blast radius 2 — affects the cascade-editing surface in admin settings, not every request; impact 2 — an admin sees a confusing runtime failure, not immediately caught). Real problem this solves: item #20 (Gemini 3.5-vs-3.6 drift) happened because nothing checks a cascade model string against OpenRouter's actual current model catalog/slugs when it's set — a typo or a deprecated/renamed model slug would silently sit in the registry until a real completion call fails. Proposed shape: on save (or as a periodic background check) in the admin cascade editor, call OpenRouter's `/models` endpoint and validate/autocomplete against the real current slug list, catching mismatches at edit time instead of at runtime-failure time. Explicit user decision: separate feature, not bundled into the current highlights/digest/chat consistency work — pick up when scheduled. | `web/lib/services/openrouter.ts` | **deferred** | `callOpenRouter` has zero current callers (verified via exhaustive grep), but ADR 011 explicitly documents it as the intentional dormant Vercel single-model-completion fallback ("chat completions, if ever used") — **do not delete**. On final-tier `AbortError` it already correctly `throw`s (not a silent swallow). No caller exists yet to catch that throw and settle UI/DB state — re-verify the day this path gets wired up, not before. |
