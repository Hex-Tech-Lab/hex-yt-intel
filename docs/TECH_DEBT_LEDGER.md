...
| 6 | 17 | Supabase logs `logs.all` endpoint is deprecated; its successor `/logs` (ClickHouse SQL) is currently broken for `postgres_logs` | `web/lib/admin-logs/fetchers.ts` (`fetchSupabaseLogs`) | open | Found via `/stress-test` decision analysis (OC/DeepSeek, 2026-08-03) while investigating the dead-since-creation Logs tab bug (item resolved separately). Live-verified: `GET /logs?sql=select...` against `postgres_logs` returns `{"error":"Backend error! Retry your query."}`; Supabase's OpenAPI spec marks `logs.all` `"deprecated": true`. Risk: if `.all` is removed upstream without notice, this recreates the exact silent-dead-tab bug that already happened twice this session (HTTP-200-with-empty-result or now a hard error, no alert). Recommended next step (not yet done): live-curl `GET /logs?sql=select event_message from edge_logs limit 5` to determine whether `/logs` is broken entirely or just for the `postgres_logs` table specifically (different ClickHouse-native table, may work). If it works, file a follow-up to migrate with a ClickHouse-SQL rewrite (column names may differ from the Logflare-backed `.all` schema, not yet confirmed identical). If `edge_logs` also fails, this is a Supabase-side outage/misconfig, not a code fix — open a Supabase support ticket instead. |
