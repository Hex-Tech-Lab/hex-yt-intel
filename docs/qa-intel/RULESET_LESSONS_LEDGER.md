...
## 2026-09-07 — `credential-leak-detector` (CredentialLeakRule) has no test-file exclusion, blocking PR #286 CI

- **Rule**: `credential-leak-detector` (`scripts/quality-engine/rules/security.ts:5-26`)
- **Symptom**: flagged `web/lib/usecases/__tests__/get-user-entitlements.test.ts:170` as "Security: Hardcoded Sensitive ID" (critical), failing the Lint job on PR #286's CI run (34065761329) and blocking merge.
- **Root cause**: the rule hardcodes `FORBIDDEN_IDS = ['test-user-id', 'da4381c6-f774-4c99-8f04-2c1c9e27d1fb']` — that exact UUID was the real finding from the original PR #286 security review (a hardcoded admin-bypass ID in production logic) — but the rule has no path guard excluding `__tests__`/`.test.`/`.spec.` files, so any test fixture that happens to reuse that literal string as an arbitrary test-user UUID trips the same "critical" finding as a real production bypass, even though a hardcoded ID in a unit test fixture is not the same risk class.
- **Fix**: added a test-file path exclusion (`/\.(test|spec)\.[tj]sx?$/` or `__tests__/`) mirroring the existing exclusion pattern already used for the rule's own quality-engine files two lines above. Verified the exclusion doesn't mask the rule's real intended target — a non-test file containing either forbidden literal still fires.
- **File**: `scripts/quality-engine/rules/security.ts`. Unblocks PR #286.
