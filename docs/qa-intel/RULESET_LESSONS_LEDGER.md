...
- **False negative, NOT fixed, needs its own design pass**: separately, OC (Muse Spark 1.2) had added `HARDCODED_OWNER_EMAIL_PATTERNS = [/kelly/i, ...]` as a founder-entitlement bypass in `GetUserEntitlementsUseCase.ts` (commit 6ed7c492, 2026-08-28, live in prod ~10 days before caught) — an unanchored substring regex matching ANY email containing "kelly" anywhere, letting an attacker-controlled email bypass billing entirely. The co-located hardcoded UUID in that same block DID trip `credential-leak-detector` and was consciously accepted at the time (`.memory/AGENT_LEDGER.md`: "qa 1 low (intentional hardcoded owner ID per directive)") — but the far more dangerous unanchored-regex email pattern next to it was never flagged by any rule, so it was never separately surfaced as its own decision point.
- **Root cause**: no rule in the ruleset detects "a regex literal used in an authorization/entitlement decision that is not word-boundary/exact-match anchored against user-controlled input (email/username)" as its own finding class.
- **Not fixed at the rule level** — needs a real design pass, not a quick pattern hack: a naive anchoring check (require `^`/`$`) would still pass `^kelly` matching `kellyattacker@evil.com`. The correct rule shape is closer to "flag any `RegExp` literal whose `.test()`/`.exec()` result gates a branch returning elevated entitlements, unless compared via exact-match against an env-var-sourced or constant allowlist" — genuinely needs a rule author's design time, flagging here per the mandatory ledger process rather than shipping a hack under time pressure.
- **Real code fix**: removed the whole hardcoded fallback block in PR #288 — legitimate founder access already has a safe, exact-match, env-var-driven path (`FOUNDER_USER_IDS`/`ADMIN_FOUNDER_EMAILS`) immediately below it in the same file.
