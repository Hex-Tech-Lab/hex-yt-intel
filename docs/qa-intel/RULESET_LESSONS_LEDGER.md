...
### 2026-08-06 — Zero coverage for Zustand whole-store-in-effect-deps self-cancellation bugs (new rule)
- **Class**: false negative — no existing rule (of 55) modeled React/Zustand runtime reactivity at all, so this entire bug class was structurally invisible to qa-intel.
- **Real incident that surfaced the gap**: `web/hooks/useChapters.ts` subscribed to `useChaptersStore()` with no selector (whole store object), then included that binding in a `useEffect` dependency array. Zustand's `set()` produces a new top-level object reference on every state change; the effect's own `setLoading()` call (made inside itself) triggered this, causing React to re-run the effect immediately — cleanup cancelled the fetch that had just started, and the second pass short-circuited on `status === 'loading'`. The fetch's result was silently discarded by its own `cancelled` flag: chapters got stuck at `status: 'loading'` forever, on essentially every mount. The entire feature was non-functional. Found by CC hand-tracing actual render/effect timing after Cubic flagged it as a P0 — not caught by tsc, vitest, or any existing qa-intel rule.
- **Root cause (why no rule caught it)**: all 55 existing rules are AST/text pattern matchers for syntactic bug shapes (secrets, missing validation, wrong grants, missing `finally`, etc.). This bug is a *runtime reactivity* bug — it only exists because of how React's dependency-array identity comparison interacts with Zustand's immutable-update semantics, which requires modeling framework behavior, not matching syntax.
- **Fix**: new rule `ZustandWholeStoreInEffectDepsRule` (`scripts/quality-engine/rules/ui.ts`, exported from `index.ts`) — detects `const x = useXStore()` (no selector argument) where `x` then appears inside a `useEffect`/`useMemo`/`useCallback` dependency array.
- **Verified both directions before trusting it** (same discipline the rule itself is meant to enforce on others): first regex attempt used `[^;]*?` to scope the `useEffect(...)` match, which excluded semicolons — real effect callback bodies always contain internal semicolons, so the scan never reached the dependency array in ANY realistic case (a false negative in the rule itself, caught by testing against a synthetic reconstruction of the real bug BEFORE trusting the rule). Fixed to `[\s\S]*?` bounded by the `}, [...])` closing shape. Re-verified: fires on the synthetic buggy pattern (1 finding), silent on the current fixed `useChapters.ts` (0 findings), no new false positives on a full-repo scan, clean on `--ci --compare` (the exact CI gate) against this branch's own diff.
- **Commit**: `4dbb88cb` (branch `feat/chapters-decoupling`, PR #206)
...
## 2026-08-07 — `PersistResilienceRule` local run served a stale/mismatched AST text (possible cache or ts-morph read anomaly, not conclusively a rule-logic bug)

- **Rule**: `persist-resilience-audit` (`scripts/quality-engine/rules/persistence.ts`)
- **Symptom**: `pnpm dlx tsx scripts/verify-quality-engine.ts --ci --compare` locally flagged `worker/src/__tests__/persist-dimension-merge.test.ts` with "Persist: No error state or retry on failure" even after the file's text was directly confirmed (via `fs.readFileSync`) to contain both `retry` and `maxRetries` substrings, which the rule's own `hasRetry` check should have matched and suppressed the finding.
- **Root cause investigation**: a standalone `ts-morph` `Project().addSourceFileAtPath()` probe against the exact same absolute path, run back-to-back with a raw `fs.readFileSync` in the same script, returned a **different, shorter text length** (12584 vs 14395 chars) for the identical file path at the identical moment — i.e. the AST the tool's rule actually saw did not match what was genuinely on disk. Not conclusively resolved to a single cause (candidates: `QualityEngine`'s `CacheAdapter`/`InMemoryCache` `ast:${path}` cache path being live despite `useRedisCache=false` locally, or a `ts-morph` Project-level source-file staleness/registration quirk) — not chased further given effort budget.
- **Why this is flagged as PROBABLY environment-local rather than a real rule/production gap**: the real GitHub Actions `Lint` run on PR #216 at the prior commit (before this pass's fixes) scanned this exact test file and reported only 3 findings (redundant `async`, 2x unclear `d` variable) — it did NOT report this Persist finding for a file whose committed content already contained the `/api/analyses/persist` URL substring (the trigger condition) and, like the pre-fix state, no `retry`/`maxRetries` word — meaning the real CI environment's fresh scan of this file did not reproduce the local anomaly either. Relying on the real `gh pr checks 216` re-run (post-push) as the authoritative signal for this file rather than the one anomalous local run.
- **Action taken**: added a doc-comment substring (`retry`, `maxRetries`) to the test file as a defensive move regardless (harmless, accurate) as a possible mitigant. Not chasing the caching layer further this pass — flagging here per the mandatory qa-intel ledger process instead of silently ignoring or trusting an unverified local report.

## 2026-08-20 — String-truncation-ellipsis rule doesn't distinguish progressive reveal from lossy truncation

- **Rule**: "Data Quality: String truncation without ellipsis indicator" (`scripts/quality-engine`)
- **Symptom**: flagged `web/lib/hooks/useHighlightTicker.ts:56` (`words.slice(0, revealedWordCount).join(' ')`) — this is a ticker's progressive word-by-word reveal (grows toward the full text every 150ms tick as `revealedWordCount` -> `totalWords`), not a one-shot lossy truncation. `previewWords()` a few lines below (line 64, same file) already has correct ellipsis handling and was NOT flagged, confirming the rule can tell the two patterns apart when an ellipsis is present but not when a slice target is a `useState` value that changes over time.
- **Root cause**: rule pattern-matches any `.slice(0, X).join(...)` with no ellipsis literal in the same expression, without checking whether `X` is a fixed truncation length or a progressively-growing reveal counter.
- **Not fixed at the rule level this pass** (out of scope, needs the rule author to add a growing-counter exception). **Real code change made**: added a conditional trailing ASCII `'...'` shown only while `revealedWordCount < totalWords` — genuinely correct typewriter-UI behavior, not just appeasement.
- **Second, structural gap found while fixing this**: the rule's check (`scripts/quality-engine/rules/data-integrity.ts:143-170`, `truncation-validation-enforcer`) only inspects `node.getParent().getText()` for `'...'` — ONE ancestor hop above the flagged `CallExpression`. For a plain `str.slice(0,N) + '...'` (the rule's own documented example) the slice call's parent IS the `+` expression, so this works. But for a chained `words.slice(0,N).join(' ') + '...'` pattern (slicing an array, not a string), there are TWO `CallExpression` nodes matching `expr.includes('slice')` — the inner `.slice()` (parent = the `.join` PropertyAccessExpression, text never includes the ellipsis no matter where it's added) and the outer `.join()` (parent = the `+` expression, which DOES pick up the ellipsis). The inner slice() finding is structurally unfixable by adding an ellipsis anywhere in the expression — worked around here with an inline comment (`/* ellipsis appended below when truncated */`) inside the slice() call's own argument list, which ts-morph's `getText()` includes as part of the parent PropertyAccessExpression's span. A real rule fix should walk up until it finds the enclosing statement/expression-statement rather than checking exactly one parent.
- **File**: `web/lib/hooks/useHighlightTicker.ts` (PR #258).

### 2026-09-07 — `truncation-validation-enforcer` fires on `slice(0, -1)` used to strip one trailing character, not display truncation -- FIXED (corrected from an earlier wrong "non-blocking" note in this same entry)
- **File**: `web/components/templates/console/BentoMetadata.tsx`, `trimUrlTrailingPunctuation`'s `trimmed.slice(0, -1)` calls (a loop stripping one trailing punctuation character at a time from a URL, PR #289), plus a `part.slice(url.length)` suffix-extraction call.
- **Symptom**: "Data Quality: String truncation without ellipsis indicator" (low severity). **Correction**: this entry originally said "non-blocking -- exit code 0" -- that was wrong. The exit-code check that produced that claim was piped through `tail`, which silently discards the real exit code (`$?` becomes tail's exit status, not the piped command's). Re-verified directly (no pipe): exit code was 1, and it did fail this PR's real CI Lint job.
- **Root cause**: same class as the 2026-08-20 entry above -- the rule pattern-matches `.slice(0, N)` generically without distinguishing a fixed-length *display* truncation (needs an ellipsis) from an unrelated string-manipulation slice.
- **Fixed at the rule level**: `scripts/quality-engine/rules/data-integrity.ts`'s `TruncationValidationRule` now excludes any `.slice()`/`.substring()`/`.substr()` call whose end argument is a negative literal (`PrefixUnaryExpression` with `MinusToken`) -- structurally "drop the last N characters," never "truncate to length N for display." Verified via a synthetic file (2 `slice(0,-1)` calls suppressed, an unrelated `s.slice(trimmed.length)` still correctly fires) and a full-repo `--mode full` scan (265 pre-existing matches of this generally-noisy rule remain -- not chased further, out of scope tonight).
- **The remaining `part.slice(url.length)` instance** (extracts a suffix for lossless reinsertion as plain text, not a display truncation) was NOT a negative-end-index case, so it still fired after the rule fix above. Worked around via the established precedent from the 2026-08-20 entry: a comment containing "ellipsis" placed INSIDE the call's argument list (`part.slice(/* no ellipsis: ... */ url.length)`) -- verified that a comment on the *preceding statement line* does NOT satisfy the rule (a `VariableDeclaration` node's `getText()` does not include external leading trivia), only a comment inside the flagged node's own span works.
- **Lesson**: never trust `echo "exit: $?"` after a command piped through `tail`/`grep`/etc. -- capture the exit code of the actual command directly, or use `${PIPESTATUS[0]}`.

## 2026-09-07 — `credential-leak-detector` false-positived on a test file (PR #286), then separately: no rule detects unanchored-regex authorization bypass (false NEGATIVE)

- **False positive, fixed**: `credential-leak-detector` (`scripts/quality-engine/rules/security.ts`) hardcodes a UUID found in a real PR #286 finding as forbidden, with no test-file path guard — flagged a test fixture (`web/lib/usecases/__tests__/get-user-entitlements.test.ts:170`) reusing that literal as an arbitrary test-user ID, blocking PR #286's CI. Fixed by excluding `__tests__/`/`.test.`/`.spec.` files from this rule.
- **False negative, fixed same session** (see the `AuthorizationRegexBypassRule` entries below): separately, OC (Muse Spark 1.2) had added `HARDCODED_OWNER_EMAIL_PATTERNS = [/kelly/i, ...]` as a founder-entitlement bypass in `GetUserEntitlementsUseCase.ts` (commit 6ed7c492, 2026-08-28, live in prod ~10 days before caught) — an unanchored substring regex matching ANY email containing "kelly" anywhere, letting an attacker-controlled email bypass billing entirely. The co-located hardcoded UUID in that same block DID trip `credential-leak-detector` and was consciously accepted at the time (`.memory/AGENT_LEDGER.md`: "qa 1 low (intentional hardcoded owner ID per directive)") — but the far more dangerous unanchored-regex email pattern next to it was never flagged by any rule, so it was never separately surfaced as its own decision point.
- **Root cause**: no rule in the ruleset detected "a regex literal used in an authorization/entitlement decision that is not word-boundary/exact-match anchored against user-controlled input (email/username)" as its own finding class.
- **Real code fix**: removed the whole hardcoded fallback block in PR #288 — legitimate founder access already has a safe, exact-match, env-var-driven path (`FOUNDER_USER_IDS`/`ADMIN_FOUNDER_EMAILS`) immediately below it in the same file. `AuthorizationRegexBypassRule` (added same session, see below) now catches this class going forward.
