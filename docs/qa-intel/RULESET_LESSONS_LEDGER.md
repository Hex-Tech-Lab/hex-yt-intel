...
### 2026-09-07 — `truncation-validation-enforcer` fires on `slice(0, -1)` used to strip one trailing character, not display truncation
- **File**: `web/components/templates/console/BentoMetadata.tsx`, `trimUrlTrailingPunctuation`'s `trimmed.slice(0, -1)` calls (a loop stripping one trailing punctuation character at a time from a URL, PR #289).
- **Symptom**: 2x "Data Quality: String truncation without ellipsis indicator" (low severity, non-blocking -- exit code 0).
- **Root cause**: same class as the 2026-08-20 entry above -- the rule pattern-matches `.slice(0, N)` generically without distinguishing a fixed-length *display* truncation (needs an ellipsis) from an unrelated string-manipulation slice (stripping a known suffix character, here `-1` specifically, which is never a display-truncation length).
- **Not fixed at the rule level this pass** (non-blocking, low priority relative to the rest of this session's queue). A `slice(0, -1)` (negative end index) specifically is never a truncation-for-display pattern and could be a cheap, safe exclusion for the rule.
