## 2026-09-19 STATUS — reconciled against `docs/history/ROSTER_2026-07-20.md` and everything merged since PR #308

**Method**: every status in this block was re-checked from source, `git log`, or `gh` on 2026-09-19 (`main` @ `313a4c51`), not carried forward from the sections beneath. **"Not verified" means no evidence either way was found — it does not mean fine.** Nothing here was checked live in production. The launch-shape correction in the 09-10 block below still stands (we are NOT live; ~1-week founder pre-sign-up). §0's dates (08-24 deadline, 08-26 stoppage) and "Suggested sequencing" are historical and were deliberately not rewritten.

**Repo state**: 0 open PRs, 0 open GitHub issues.

### A. Changed since the 09-10/11 re-audit

| Item | Was | Now | Evidence |
|---|---|---|---|
| Plaintext test-account credential in `testsprite_tests/TC*.py` (found 09-10, flagged in the 09-10 block below) | open, unowned | ✅ code side done: scripts read `TESTSPRITE_TEST_ACCOUNT_EMAIL`/`_PASSWORD` from env and fail at import if unset; no hard-coded password pattern remains in tracked `TC*.py`. Password was rotated out-of-band 09-10 *(ledger claim, not re-verified)*, so the value still sitting in git history is no longer live | PR #316 `291e91d8`; `git grep` clean |
| Next.js RCE CVEs (Codacy Wave A-security, item 1) | untracked | ✅ `next` 16.2.11 → 16.3.3, plus a Vercel/Turbopack `output: 'standalone'` fix the upgrade required. **Not verified**: that 16.3.3 covers both CVEs, or that Codacy's dashboard shows them closed | PR #317 `9fac1671` |
| §9.10 persist-route test | ⬜ zero tests | 🔵 substantial: finalize CAS, chunk-payload classification, side-effect outbox, primitive-payload tests landed. **Not found**: the specific gate assertion (a partial result keeps `validationPassed`/`billing_status` consistent) | PR #314 `fd641385`; `web/lib/__tests__/persist-*.test.ts` |
| §9.11 / §9.12 adapter tests | ⬜ | ⬜ still no dedicated `SupabasePersistenceAdapter` / `SupabaseAnalysisAdapter` test file | `find web` |
| Stream resilience + persist hardening (never on this checklist) | — | merged: connection-drop resilience (fetch timeout now covers body consumption, unified retry budget — #313 → #315); atomic parent-finalize CAS + malformed chunks classified terminal-failed (#312 → #314). Both bear on §6.6's smoke test | `94bf0856`, `fd641385` |
| Dim 2.3/4.2 hallucinated-named-expert gap | — | ✅ closed by PR #318 (`credibility-grounding.ts`). The ledger also claims Dim 8.4 was fixed; the 09-19 handover found no such change in the diff — treat that half as **unverified** | `bae91c21` |
| Dimension flyout copy button | — | ✅ shipped and confirmed visible by the user (needed a hard refresh to clear a stale cache) | PR #319 `35c3f772` |

### B. Unchanged since 09-10 (re-verified from source today)

- **§2.1 Paddle**: `web/lib/paddle.ts` still defaults to `Environment.sandbox`; `PADDLE_API_KEY_LIVE` is referenced nowhere under `web/lib` or `web/app`. Prod env value not inspected. Real charges still cannot happen.
- **§2.5 founder checkout**: `web/app/founders/page.tsx:18` still carries `// Do NOT wire real checkout/billing here until numbers are final.`
- **§5.2 T&C footnote**: `docs/legal/terms-of-service.md` still has **zero** mentions of GDPR, retention, Supabase, Cloudflare, OpenRouter, Paddle or Upstash. Ask still unmet.
- **§6b load/stress tests**: no load, stress or concurrency artifact is tracked in the repo.
- **§1e.1 / #241 cascade SSOT**: issue closed 08-20; a code comment in `worker/src/services/LLMCascade.ts` says the hardcoded `['anthropic','google-vertex','amazon-bedrock']` list was removed. The gate (a live OpenRouter request log showing the right provider) was **not** re-run.
- **§3.3 / §4 / §2d marketing, intro video, teasers**: no repo artifact exists; these are creative, user-owned items and can't be verified from source.
- **Not re-checked, no commit touches them since 09-11**: §1b.5–1b.7, §1c (export quality), §1d (chat escalation tier), §2d.1–2d.2, §3.4–3.6.

### C. New items this reconciliation surfaced (not previously tracked here)

| # | Item | Status | Why it matters for launch | Gate |
|---|---|---|---|---|
| 10.1 | **Paddle webhook path + tier vocabulary reconciliation.** Two Paddle webhook routes exist. The legacy `web/app/api/billing/webhook/route.ts` (still allow-listed in `web/middleware.ts:141`) hard-codes `tier: 'pro'` on every `subscription.created/updated` and `'free'` on cancel — it cannot represent Light or Max. The newer `web/app/api/webhooks/paddle/route.ts` (08-26, entitlement-based) has no price→tier mapping in `ProcessPaddleWebhookUseCase.ts` by grep, so entitlements may replace `users.tier` — **not determined**. The tier vocabulary also disagrees across three places: `UserTier` = `free\|pro\|enterprise` (`web/lib/types/billing.ts:8`); `PriceTier` = `free\|light\|pro\|max\|founder*` (`web/lib/config/pricing.ts:34`); `retention_policies` CHECK = `free\|light\|casual\|core\|power\|pro\|enterprise\|admin` (migration `20260809122802`). `updateUserTier` is typed `'pro'\|'free'` in both port and adapter. | ⬜ **not investigated beyond the above** | A Light or Max purchase may grant the wrong tier (or none) and the wrong retention window. Same "N disagreeing sources" failure class as §1e.1. Roster flagged the `updateUserTier` half as a tangent on 07-23; it was never escalated. | Confirm which endpoint is registered in Paddle's dashboard; delete the dead route; one tier vocabulary; a sandbox purchase of each of Light/Pro/Max lands the correct tier and retention |
| 10.2 | **Wave A-security items 2–5** (Codacy, 55 findings; only item 1 shipped): `CheckoutButtonProps` open-redirect-shaped finding (**billing-adjacent; due ~4 days after 09-15, likely past its Codacy SLA**), `dangerouslySetInnerHTML` ×2 (one already ~2 months overdue at last check), File-Access/SSRF/weak-RNG clusters (~37), command-injection/prototype-pollution | ⬜ not started | Open security findings on a payment-adjacent surface before a public founder sign-up | Each finding fixed or triaged with a written verdict |
| 10.3 | Wave A (qa-intel rules from Codacy frequency list) and security bulk-triage OC prompts | 🟡 written and committed (`8ebda550` on `chore/restore-claude-files-and-crg-mcp`), **never dispatched**. Bulk-triage prompt references branch `fix/wave-a-security-backlog`, which #317 already used and merged — fix its §2 before dispatch | Feeds 10.2 | Dispatch decision |
| 10.4 | Wave B (P2–P5 backlog tree + ROI) | ⬜ not started; branch `docs/wave-b-backlog-scoping` has 0 unique commits. Scoping only until reviewed, per standing directive | — | — |
| 10.5 | **Roster live-verification backlog**: A2 (transcript), A5 (comments), A6, A7, A8, A9, A10, A11, A12, A13 were all "code-fixed, needs a live re-test" in the roster and no sign-off is recorded anywhere | ❓ **not verified** | These are exactly the flows §6.6 must prove (fresh analysis → Complete/`billing_status` correct → digest without opening the panel → transcript chat → time-seek) | Fold into the §6.6 smoke test as an explicit per-item pass list |
| 10.6 | Roster bug: on iPad an already-analyzed video showed "Analyze" instead of "Reanalyze" (07-23); did not reproduce on desktop | ⬜ never investigated | Can double-charge or lose a re-analysis | A repro, then a root cause |
| 10.7 | Roster decision: bulk re-analyze old "Partial" rows now that A6+A11 are fixed, and whether their `billing_status: 'failed'` history means they were never charged | ⬜ undecided | Billing correctness for existing users | A written decision; a check that no failed-status row was charged |
| 10.8 | Carried incidents with no record of resolution: `NE-62S4OYCg` (0/11 dims, failed twice, root cause unconfirmed, blocked on the CF Observability MCP schema bug), WordCloud 500 render site, `jsonrepair_failed` Sentry re-check now that #313's telemetry is live | ❓ **not verified** | Unexplained production failures | Each has a verdict |
| 10.9 | **Upstash Vector: dashboard shows 0 requests / 0 B / $0.00 (user-reported 2026-09-19) — CORRECTED same day with prod evidence.** **The app's index is healthy and in use; the dashboard the user looked at is almost certainly a different Upstash index/database.** Evidence (`upstash_snapshots`, the app's own QStash poll using its production credentials): 5,035 `/info` polls 2026-07-29 → 2026-09-19 19:00 UTC, **all `ok`**; a HYBRID index, 1536-d, COSINE, **50 vectors, ~1.9 MB**, count grew 10 → 50 with the latest increase on 2026-09-18 (so the embed webhook has been writing recently). A 15-min poll alone would register as dashboard requests, so a 0-request/0 B view cannot be this index. Vercel Production has `UPSTASH_VECTOR_REST_URL`/`_TOKEN` (set 118 d ago; values not inspected). **Real open problem = coverage:** 115 analyses are `completed` (109 distinct videos, 17 in the last 30 days) but the index holds only 50 vectors, so search likely cannot find roughly half or more of completed analyses. Cause **not determined** — candidates: pre-Upstash analyses never backfilled; embed webhook 503/skip/failure on some analyses (`web/app/api/webhooks/embed/route.ts`, no Sentry capture on the credential-missing branch); unembedded analyses with no retry. Fits TestSprite TC015 (08-20) returning zero results for an account with no embedded analyses — hypothesis. **Supabase pgvector is effectively unused**: 1 of 239 `analyses` rows has an `embedding` (2026-05-15, not completed) and none of the completed ones — nothing would be lost by disabling it (migration, ADR 018 rules; user asked to consider). | 🔵 **partly resolved**: wrong-dashboard explanation is strong but not confirmed by the user; coverage gap and pgvector removal not started | Semantic search is a shipped feature; if it only covers a fraction of analyses it silently returns incomplete results | User confirms which Upstash database the app's `UPSTASH_VECTOR_REST_URL` belongs to; cause of the 50-vs-115 gap established; backfill or retry mechanism; decision + migration on dropping pgvector (`analyses.embedding`, `optimize_vector_search_rpc`, `vector` extension) |

---

## ⚠️ 2026-09-10 STATUS — corrected and re-audited, read before trusting anything below

**Correction to an earlier same-day note**: this document previously claimed the product was "already live" — **wrong, corrected directly by the user: we are NOT live.** What's real: `getvintel.com` runs real production infrastructure (real Supabase rows, real `billing_status`/quota logic, real bugs with real consequences) under active development/internal use, but there has been no public launch. Don't conflate "production infra exists and has real bugs" with "launched" again.

**Real, corrected launch shape (per the user directly, 2026-09-10)** — this supersedes §0's original framing entirely:
- Launch includes a **~1 week pre-launch founder sign-up period**, not a single go-live day. Not previously in this document.
- **Marketing material / teaser assets: not started.** (§4 below still shows this as 4 unstarted videos — that's still accurate, nothing shipped since 08-19.)
- **Landing page / intro video recording: not done.** Same gap as 08-19, unchanged in 3.5 weeks.
- **Founder page (`/founders`) exists** but has an accent-color mismatch under active discussion (green "discount honored" text vs. this product's frozen cyan palette; user is now considering orange instead — undecided, see §2c.2 new sub-item below). Do not change the live page without an explicit go — a copy-first workflow was requested.
- **Payment stack: not finalized**, but genuinely more progress than the 08-19 audit showed (see §2 real update below) — the OLD "zero forward motion" framing is now wrong and replaced.
- **Runway**: user was on annual leave, returned 2026-09-06, ran ~1-2 hrs/day through the return week per the 2026-09-07 note; **as of today, back to ~4-8 hrs/day.** Recalibrate scope of proposed next steps upward accordingly — the 1-2hr/day constraint that gated recent sessions no longer applies.

**Real re-audit done this session (2026-09-10), evidence attached inline in §1/§2/§6 below** — not a guess, verified via `gh pr view`/`gh issue view`/`git log`:
- §1.2 (PR #239 merge): **confirmed MERGED** 2026-08-19 — was still showing ⬜ in the stale version of this doc.
- §6.3 (issues #241/#242/#243): **all 3 confirmed CLOSED** 2026-08-20 — was showing as open findings in the stale version.
- §2 (Paddle): **real, dated progress found that the 08-19 audit missed entirely** — commits `85eb4eea` (checkout session creation + entitlement evaluation) and `2b5d497d` ("complete paddle merchant of record integration (phases 1-3)"), both dated **2026-08-26**. The 08-19 checklist's "§2 has had zero real forward motion" verdict is now WRONG and superseded — see §2 below for what's actually still open.
- **OC/GLM-5.3-flash-low dispatch completed and independently spot-checked** (`docs/PRE_LAUNCH_AUDIT_RAW_2026-09-10.md` has the full raw evidence; 2 of its most consequential claims re-verified directly by CC, both confirmed accurate):
  - §2.1 (Paddle live/sandbox): **`PADDLE_ENVIRONMENT=sandbox`** confirmed directly — still sandbox, `PADDLE_API_KEY_LIVE` exists as an env var name but is referenced nowhere in `web/`. Real charges cannot happen yet regardless of the phases 1-3 integration work.
  - §2c (Simple/Pro): **confirmed fully shipped and wired** — `ViewModeToggle`/`useConsoleViewStore`/`useEffectiveViewMode` all present and connected in `DashboardContainer.tsx`/`TopBar.tsx`, matching the 2026-08-26 ledger entries. No longer an open item.
  - §3.1/3.2 (waitlist): **confirmed live and functional** — real `<form onSubmit>` posting to `/api/waitlist`, 289 lines, dated 2026-08-26.
  - §5 (GDPR/legal): **real pages exist** (`docs/legal/privacy-policy.md` has genuine GDPR/CPRA sections + a Data Protection Officer contact + the 24-72h retention disclosure) — **but `terms-of-service.md` has ZERO mentions of GDPR, retention, or any infra provider** (Supabase/Cloudflare/OpenRouter/Paddle) despite `sub-processors.md` correctly listing all 8. The original §5.2 ask (footnote on infra/retention IN the T&C) is not actually satisfied — the disclosure landed in the privacy policy instead, not the T&C.
  - §6b (load/stress tests): **confirmed still not run** — no artifact, ledger entry, or file newer than 08-19 shows a 5-hour-video or 50-concurrent test ever executing.
  - §7 (TestSprite): **a NEWER, worse result was found that this checklist never recorded** — `testsprite_tests/tmp/raw_report.md` is dated **2026-08-20** (not the 08-19 run this checklist §7.1 describes), showing only **3/15 passed (20%)**, down from the 08-19 run's already-mixed 2 passed/2 failed/11 blocked. Real new failures in the 08-20 report: `"Invalid appUrl callback destination"` breaking bundle streams (TC001/TC002) and search returning zero results (TC015). This 08-20 run and its worse outcome were **never reflected anywhere in this checklist until now** — §7.3's "~75-78% confidence" verdict was computed from the earlier, better-looking run.
  - **Real, unrelated security finding surfaced during this audit**: 11 git-tracked `testsprite_tests/TC*.py` files contain a plaintext test-account credential (email + password) committed to the repo — confirmed present (not printing the value). Should be rotated and removed from git history, not just the working tree, as its own follow-up — flagging here since nothing else in this document was tracking it.
  - Founder checkout: **more scaffolding exists than assumed** — `web/lib/config/pricing.ts` already has placeholder `founder_tier_a`/`founder_tier_b` Paddle sandbox price IDs ($49/$99) wired through the generic `/api/billing/checkout` route's price resolution. The page-level "do not wire real checkout" instruction is still respected (the founders page itself doesn't call it yet) — this is closer to done than §2.5 above implied, worth a fast finish rather than a from-scratch build.
  - Marketing/teasers: **confirmed zero files** — nothing in `web/public/` or `docs/` newer than 08-19 is a marketing asset (only product screenshots and research docs).

**§9** (monolith decomposition + zero-test-coverage backfill), further below, added this session in response to a real bug (`validation_passed` misreporting partials as `'completed'`) found live in production and traced to an untested 1158-line file — still accurate, unchanged.
...
| 1.2 | Merge PR #239 | ✅ **verified MERGED** (`gh pr view 239` → `mergedAt: "2026-08-19T15:56:09Z"`, re-checked 2026-09-10) — the "still OPEN" status below is stale, from the 08-18 audit before it merged | CC | done | Merged to main — **not yet independently re-verified live in prod this session**, only merge status confirmed |
...
**2026-09-10 real re-audit — supersedes the 08-19 "zero forward motion" verdict entirely.** `git log --since=2026-08-19 -- web/lib/paddle.ts web/lib/billing-factory.ts` shows real, dated commits the last audit missed: `85eb4eea` (checkout session creation + entitlement evaluation use case) and `2b5d497d` ("complete paddle merchant of record integration (phases 1-3)"), both **2026-08-26**. §2.2 below is likely done or close to it — needs a live-flow re-verify, not a full rebuild.

| # | Item | Status | Owner | Effort | Gate |
|---|---|---|---|---|---|
| 2.1 | Confirm Paddle account status (already primary per prior decision — verify not just "selected" but actually provisioned: API keys live, webhook endpoint registered, sandbox checkout tested) | ⬜ **not re-verified this session** — `web/lib/paddle.ts` reads `PADDLE_API_KEY` from env generically; whether that's currently pointed at sandbox or the live key (`PADDLE_API_KEY_LIVE` per the 08-16 hand-off, deliberately not wired in yet as of that note) was not checked this pass — real env values weren't inspected | CC verify, you provision (KYC not agent-doable) | 30m verify | Real Paddle checkout completes in sandbox, webhook received and logged |
| 2.2 | Wire Paddle into the existing modular MoR abstraction | 🔵 **real progress found, not previously tracked** — commits `85eb4eea`/`2b5d497d` (2026-08-26) claim "phases 1-3" complete. **Not independently re-verified this session** (no live checkout run, no code read of the actual adapter this pass) — flagging the finding, not the verdict | CC to verify next | 30-60m verify (down from the original 2-3h build estimate, since it may already be built) | Subscribe/cancel/webhook flows hit Paddle through the abstraction; CC verifies no direct Paddle SDK import outside the adapter |
| 2.3 | Decide: 1 MoR (Paddle only) for launch vs. 2-3 | 🔵 **new candidate ruled out this session**: "Payments AI" (payments.ai, ClickFunnels' native gateway) confirmed via their own docs to support only US/CA/UK/DE/IT/ES — Egypt not reachable, not viable for this case. Dodo Payments (candidate #2 from 08-16 research) remains unconfirmed — still needs direct confirmation now that there's real bandwidth to do it | you decide, CC can confirm Dodo | 30m to confirm Dodo Egypt support | Recommend Paddle-only for launch, confirm Dodo as the real fallback candidate before dropping it |
| 2.4 | End-to-end live payment test (real card, real charge, refund it) | ⬜ | you (needs a human with a real card) | 15m | One real successful charge + refund visible in Paddle dashboard |
| 2.5 | **NEW (2026-09-10)**: founder pre-sale checkout mechanism — the actual conversion path for the ~1 week founder sign-up window | ⬜ **confirmed still not built**, deliberately: `web/app/founders/page.tsx` contains the explicit comment `// Do NOT wire real checkout/billing here until numbers are final.` | — | blocked on final founder pricing lock (§1b.5) | Real checkout path tested end-to-end with a founder-tier price, before the founder-week countdown starts |
...
## 2d. Founder pre-sale week + launch marketing (NEW, 2026-09-10 — real structural change, not previously tracked)

**Why this is new**: the launch shape changed today — it now explicitly includes a ~1 week pre-launch founder sign-up period before general availability, not a single go-live day. None of the assets that period depends on exist yet.

| # | Item | Status | Owner | Effort | Gate |
|---|---|---|---|---|---|
| 2d.1 | Founder pre-sale week structure (start date, duration, what happens at the end — auto-convert to general pricing? hard cutoff?) | ⬜ **needs your call** | — | — | A written, dated plan for the week, not just "about a week" |
| 2d.2 | Founder page accent color — green "discount honored" text currently breaks the frozen cyan-only palette; user is now considering orange as a possible full-system accent change, undecided | 🔵 **in progress** — screenshots captured for comparison (`docs/for_sharing/2026-09-10-founder-page-accent-review/`), real WCAG contrast check for an orange candidate offered, not yet run | CC (contrast check), you (final color decision) | 15-30m for the contrast check | A concrete orange (or reverted-to-cyan) hex value with a confirmed AAA contrast ratio against the dark background, before any live page edit — copy the page first, never edit the live original directly per explicit instruction |
| 2d.3 | Marketing material / teaser assets for the founder-week launch | ⬜ **not started** | you (creative) + CC/OC for copy drafts | unscoped | At minimum: 1 teaser asset ready before the founder week countdown starts |
| 2d.4 | Landing page / intro video recording | ⬜ **not started**, same gap as 08-19 (§3.3/§4.1), unchanged in 3.5 weeks | you | — | Video file ready, hosted |
| 2d.5 | Founder pre-sale checkout mechanism | ⬜ see §2.5 above — same item, cross-referenced here since it's the hard blocker for 2d.1 actually starting | — | — | see §2.5 |
...
| # | Item | Status | Owner | Effort | Gate |
|---|---|---|---|---|---|
| 9.10 | Real test file for `persist/route.ts`'s chunked-finalize branch (the exact code path the `validation_passed` bug shipped from) | 🔵 **2026-09-19**: PR #314 added finalize-CAS, chunk-payload-classification, outbox and primitive-payload tests; the specific `validationPassed`/`billing_status` partial-result assertion was not found | CC/OC | 1-2h | At minimum: a test proving `validationPassed`/`billing_status` stay consistent for a partial (1-10/11) result, matching the fix in PR #306 |
| 9.11 | Real test file for `SupabasePersistenceAdapter.updateAnalysisResult` | ⬜ **2026-09-19**: still no dedicated adapter test file | CC/OC | 1h | Covers the guarded-write race behavior and the `validation_passed` DB-column write path |
| 9.12 | Real test file for `SupabaseAnalysisAdapter.getUserHistory`'s status mapper | ⬜ **2026-09-19**: still no dedicated adapter test file | CC/OC | 30m | Locks in the fixed billing/validation_passed precedence so this exact bug class can't silently regress |
