...
| 021 | 2026-08-02 | Granular Partial-Resume & Reaper Role Extension (scoping doc `docs/specs/ADR_021_GRANULAR_PARTIAL_RESUME_AND_REAPER_2026-08-02.md`) — one rule (piece present → leave alone, missing → refetch only that piece) applied uniformly across dimensions/metadata/transcript/digest/comments instead of today's all-or-nothing 5-bundle abort; extends the reaper's existing finalize/discard decision to a third outcome (requeue-partial) at sub-row granularity. **Phase 1 (dimension-level persistence) implemented 2026-08-07**: `worker/src/routes/analysis.ts`'s `onFragment` now captures each dimension BracketBuffer confirms during streaming; `PersistService.persist()` merges those captured dimensions with the whole-text extraction so a single malformed trailing dimension no longer zeroes out an entire chunk's already-valid dimensions (live Supabase data confirmed this exact gap). No new migration/Settings Registry key needed — existing `analysis_chunks` per-chunk durability + `BracketBuffer`'s incremental parser already covered it once wired together. Phases 2-4 (presence-check-on-resume, reaper extension, selective client dispatch) still pending. | 🔍 |
