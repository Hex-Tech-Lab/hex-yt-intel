...
| 022 | 2026-08-06 | Per-Mention Entity Timestamp Resolution (`docs/specs/ADR_022_PER_MENTION_ENTITY_TIMESTAMP_RESOLUTION_2026-08-06.md`) — `findEntityTimestamp` always resolved to the FIRST occurrence of an entity's label in dimension content, so every panel's entity-click seek landed on the same timestamp regardless of which occurrence/word was actually clicked (live-reported "entity clicks all jump to the same place" + a related WordCloud cross-highlighting bug, same root cause). v1 scoped text-layer-only (`findAllEntityMentions`/`findNearestEntityMention`, nearest-to-playhead selection) — no schema/worker/pipeline change, no video processing. Implemented + merged (PR #208), including a post-review fix pass (unthrottled playback polling, whole-store render fan-out, missing ENDED-state handling, occurrenceIndex bug). | ✅ |
| 023 | 2026-08-06 | Reliable Client-Side Knowledge Graph Fallback for Legacy/Gap Rows (`docs/specs/ADR_023_CLIENT_SIDE_KNOWLEDGE_GRAPH_REGENERATION_2026-08-06.md`) — live DB query confirmed some complete analyses (`dimension_count=11`) have NO knowledge graph data anywhere (`analysis_payload->knowledgeGraph` empty AND `kg_entities` empty); `kg_relations` is empty across the entire database. `useKnowledgeGraph.ts`'s client-side TF-IDF fallback is the only recourse for these rows but doesn't reliably display on restore — root cause not yet confirmed (candidate: effect-timing/retry gap). Approved, dispatched to OC, not yet implemented. | 🔍 |
| 024 | 2026-08-06 | Component/Hook Runtime Test Coverage (`docs/specs/ADR_024_COMPONENT_HOOK_RUNTIME_TEST_COVERAGE_2026-08-06.md`) — 3 real runtime-behavior bugs this session (useChapters self-cancellation, WordCloud repaint/ARIA staleness, digest-not-syncing-to-store) were undetectable by qa-intel/contract-auditor (static pattern matchers) and by the existing test suite (vitest.config.ts's include glob matches no `.tsx` files and no jsdom/happy-dom is installed — no test can render a real component). Adds happy-dom + React Testing Library, fixes the config gap, 4 scoped regression tests (one per already-found bug class). Additive to the existing gate stack, not a replacement. Approved, dispatched to OC, not yet implemented. | 🔍 |
