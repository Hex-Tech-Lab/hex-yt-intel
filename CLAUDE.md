...
- **OC (opencode / GLM-5.3-flash via OpenRouter, provider CoreWeave with Relace fallback, reasoning `low` — standard set 2026-09-24, see "OC model standard" below)**: **preferred default for both investigation and execution** on well-scoped findings (bug reports, PR review comments, tech-debt roster items) — do NOT pre-derive the root cause before handing off; give OC the raw finding plus exact context (file:line where already known, links, error text) and let it investigate. Prompts to OC still follow the same handholding/detailed format as AGY prompts (full context, skill stack, gates, structured report) — delegating investigation is not license to write a terse prompt.
- **GCW (Gemini Web)**: escalation path for tool failures, not a primary executor.

**OC model standard (2026-09-24, user directive — applies to every OC dispatch by any agent)**: model `openrouter/z-ai/glm-5.3-flash`, OpenRouter provider **CoreWeave first, Relace fallback** (`order`/`only: ["coreweave","relace"]`, `allow_fallbacks: true`; user directive 2026-09-24 after a CoreWeave upstream rate-limit stalled all OC jobs), `reasoning_effort: "low"` (the long-standing setting with a proven agentic/coding track record; reasoning cannot be disabled on this model — "none" is rejected by the endpoint — and "minimal" was not adopted without evidence it performs as well). Why: CoreWeave measured ~2.6x faster than Relace for ~40% more cost, and a single fixed provider keeps prompt caching on one provider. The config lives in two files that MUST stay identical: `~/.config/opencode/opencode.json` (global) and the committed `.opencode/opencode.json` (worktrees copy it from git — commit every change). Do not pass a different `-m` model or provider to `opencode run`. If both CoreWeave and Relace are rate-limited or timing out, stop and report — do not add other providers without the user. AGY runs Gemini/Claude models natively and cannot use this standard; it keeps its own per-task model choice.
