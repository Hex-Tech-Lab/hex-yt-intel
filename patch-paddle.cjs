...
if (!content.includes('import { z } from')) {
    content = importZod + content;
}

const customDataPreprocess = `const WebhookCustomDataSchema = z.preprocess((val) => {
...
content = content.replace('export interface WebhookPayload {', customDataPreprocess + '\nexport interface WebhookPayload {');
