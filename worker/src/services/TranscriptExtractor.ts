  ...
  async fetchChannelMetadata(channelId: string): Promise<Record<string, unknown> | null> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const bodyText = await response.text().catch((fetchError) => {
          console.error('[TranscriptExtractor] Error reading response body', fetchError);
          return '';
        });
        const truncatedBody = bodyText.slice(0, 300);
