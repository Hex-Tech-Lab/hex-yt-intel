-- History overview v9 — restore client_platform (dropped by mistake in v7/v8:
-- I built those off an outdated copy of this function and missed
-- 20260724110000_analyses_client_platform.sql, a LATER migration on the
-- same day that added client_platform to the return shape. Live regression,
-- caught by cubic web review against the actual PR). Also hardens the
-- comments-array check: `jsonb_typeof(x) = 'array' AND jsonb_array_length(x) > 0`
-- relies on AND short-circuiting left-to-right, which Postgres's own docs
-- explicitly do NOT guarantee for boolean expressions -- a CASE gives a
-- real evaluation-order guarantee instead of relying on planner behavior
...
as $$
  ...
  select
    ...
    (case
      when jsonb_typeof(w.analysis_payload -> 'comments') = 'array'
        then jsonb_array_length(w.analysis_payload -> 'comments') > 0
      else false
    end) as has_comments,
