-- History overview v8 — scope analysis_payload read to the winning row only.
-- Efficiency finding (/simplify pass, PR #178): v7 carried the full
-- analysis_payload JSONB (the entire UCISPayloadV2, including the comments
-- array -- can be tens of KB) through base/scored/agg for EVERY analysis
-- row per user, just to read it on the single "winner" row later. Moved to
-- a PK join back onto analyses, scoped to only the rn=1 winning row per
-- base_video_id (one row per distinct video, not one per analysis) --
-- purely a query-shape fix, no behavior change from v7.
