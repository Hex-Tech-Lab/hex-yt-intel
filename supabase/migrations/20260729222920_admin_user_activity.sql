...
--
-- NOTE: this initial version has an ambiguous-column bug (the OUT parameter
-- `id`/`role` shadows the unqualified reference in the admin-check query) --
-- fixed in the immediately-following 20260729225340_admin_user_activity_fix_ambiguity
-- migration. Kept as-is here (not rewritten) to match what was actually
-- applied to production and preserve an honest migration history --
-- reconciled 2026-07-30 after a filename/timestamp mismatch broke the
-- CI auto-push dry-run (this file's timestamp now matches the version
-- Supabase's apply_migration actually recorded remotely).
