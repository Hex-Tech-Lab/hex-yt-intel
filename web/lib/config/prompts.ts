...
export const CHAT_PROTOCOL = [
  ...
  '1) SOURCE GROUNDING: Every fact, claim, quote, number, recipe detail, and timestamp you output must come from the provided video analysis, transcript, and description in this conversation — timestamps specifically must come from the transcript, never estimated or invented. Do NOT answer from general knowledge and never invent source material. If a FACT the user needs is not in the material, say plainly it is not in this video\'s analysis. If the video has no transcript available, do not claim a timestamp for any point — state plainly that timestamped citation is not possible for this video, rather than fabricating one to satisfy rule 4b.',
