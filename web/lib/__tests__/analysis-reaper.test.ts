...
describe('sweepStuckAnalyses — requeue-partial wiring (ADR 021 Phase 3)', () => {
  ...
  function portChunk(index: number, covered: number[], overrides?: Partial<PortChunkRow>): PortChunkRow {
    return {
      ...
      payload: { dimensions: covered.map(n => ({ number: n, name: `D${n}`, content: `Body for dimension ${n}.` })) },
      status: 'completed',
      updated_at: null,
      ...overrides,
    };
  ...
  async function loadWithSweepMocks(opts: {
    ...
    vi.doMock('@/lib/adapters/SupabaseSettingsAdapter', () => ({
      SupabaseSettingsAdapter: {
        getRegistrySettings: vi.fn().mockResolvedValue({ 'remediation.maxRetries': 3 }),
      },
    }));
  ...
  it('requeues a would-be-failed row with 3/11 dimensions covered while retries remain', async () => {
    // One chunk covering dims 1-3: below MIN_SALVAGEABLE_DIMENSIONS (8), so
    // tryChunkRecovery falls through (can't salvage) and the markdown path
    // fails (no markdown) — but the checkpoint says 3 dimensions exist.
    const belowThresholdChunk: ChunkRow = {
      chunk_index: 1,
      status: 'completed',
      payload: { dimensions: [1, 2, 3].map(n => ({ number: n, name: `D${n}`, content: `Body for dimension ${n}.` })) },
    };
    ...
    expect(report.remediation_retry_count).toBe(2); // 1 → 2, shared ceiling counter incremented exactly once
  });
  ...
  it('still fails (unchanged) a stuck row with 0/11 dimensions covered', async () => {
    const { sweepStuckAnalyses, updateCalls } = await loadWithSweepMocks({
      ...
      portChunkRows: [portChunk(1, [], { status: 'interrupted' })], // never completed → vouches for nothing
    });
  ...
  it('counts a lost guarded requeue write as raced without falling through to the failed settle', async () => {
    const { sweepStuckAnalyses, updateCalls } = await loadWithSweepMocks({
      ...
      tryRecoveryChunkRows: [],
      portChunkRows: [portChunk(1, [1, 2, 3])],
      updateResult: { error: null, count: 0 }, // a concurrent settle/reap won the row
    });
    ...
    expect(result.failed).toBe(0);
    expect(updateCalls).toHaveLength(1); // only the (losing) requeue write — no terminal clobber
  });
  ...
  it('never consults the requeue path for a row the markdown path salvages as completed', async () => {
    ...
    const { sweepStuckAnalyses, updateCalls } = await loadWithSweepMocks({
      ...
      tryRecoveryChunkRows: [],
      portChunkRows: null, // findAnalysisChunks must never even be called
    });
