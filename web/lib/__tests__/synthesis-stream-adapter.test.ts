...
describe("SynthesisStreamAdapter", () => {
  ...
  describe("processLine - Fragment Routing", () => {
    it("should process status starting stage", () => {
      ...
      expect(
        terminalLines.some((l) => l.message.includes("Edge pipeline start")),
      ).toBe(true);
    });
    ...
    it("should process status model stage", () => {
      ...
      expect(
        terminalLines.some((l) =>
          l.message.includes("Running model cascade node"),
        ),
      ).toBe(true);
    });
    ...
    it("should process progressive JSON structured stream fragments", () => {
      ...
      const delta2 = JSON.stringify({
        type: "delta",
        content:
          '"persona":{"primary":{"id":"creator","label":"Creator","weight":0.5},"cognitiveLenses":[],"selectionRationale":""}}',
      });
    ...
    it("should enforce monotonicity in rebuildDisplayMarkdown but force updates on resets", () => {
      ...
      useAnalysisStore.getState().setAnalysis({
        ...
        analysis_markdown:
          "This is a long markdown text that shouldn't be overwritten.",
      });

      // Re-read to get the updated state reference
      expect(useAnalysisStore.getState().analysis?.analysis_markdown).toContain(
        "This is a long markdown",
      );
      ...
      expect(
        useAnalysisStore.getState().analysis?.analysis_markdown,
      ).not.toContain("This is a long markdown");
    });
    ...
    it("should preserve global state during bundle fallback that does not contain them", () => {
      ...
      adapterWithOptions.processLine(
        JSON.stringify({
          type: "status",
          stage: "fallback",
          error: "ERR_MODEL_OVERLOAD",
