...
export const useAnalysisStore = create<AnalysisState>((set) => ({
  ...
  // Bridges useExecutiveDigest's async-fetched digest back into the shared
  // store. initializeAnalysis only sets executiveDigest once, at stream/
  // restore start -- for a freshly-completed LIVE analysis the digest is
  // generated by a separate async call afterward (useExecutiveDigest.ts)
  // and previously only lived in that hook's own local React state, never
  // reaching this store. Consumers reading analysis.executiveDigest
  // directly (e.g. AnalysisHistory's "currently analyzing" card) saw it
  // populated only when restoring an already-digested historical analysis,
  // never for a fresh live one -- reported 2026-08-06 as intermittent
  // ("1 time it shows it, 2-3 times it doesn't"). Guarded on analysisId so
  // a slow/stale fetch can't clobber a newer analysis's state.
  setExecutiveDigest: (analysisId, executiveDigest) =>
    set((state) => {
      if (!state.analysis || state.analysis.id !== analysisId) return {};
      return { analysis: { ...state.analysis, executiveDigest } };
    }),
}));
