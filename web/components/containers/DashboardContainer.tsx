  ...
  useEffect(() => { // skipcq: JS-0903
    ...
    void (async () => {
      try {
        ...
        if (!res.ok) {
          console.warn('[AutoRestore] Check endpoint returned', res.status, res.statusText);
          return;
        }
        ...
        if (!data.exists) {
          console.debug('[AutoRestore] No cached analysis for videoId:', videoId);
          return;
        }
        ...
        if (!data.analysisId) {
          console.warn('[AutoRestore] No analysisId returned from check endpoint');
          return;
        }
        ...
        if (!restoreRes.ok) {
          console.warn('[AutoRestore] Get analysis returned', restoreRes.status, restoreRes.statusText);
          return;
        }
