...
function TopBarImpl({
  ...
  const { canShowUpgradePrompt } = useEntitlements();
        ...
        */}
        {exportOpen && (
          <>
            ...
            />
            <div className="hx-rise absolute top-[calc(100%_+_8px)] right-0 w-40 bg-[var(--surface-raised)] border border-[var(--line-strong)] rounded-xl p-1 shadow-[0_10px_25px_-5px_rgba(0,0,0,0.5)] z-50 flex flex-col gap-0.5">
              <button
                onClick={() => {
                  onExport?.("pdf");
                  setExportOpen(false);
