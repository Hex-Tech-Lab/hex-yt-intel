 ...
 *
 * The transient-failure retry/resilience tests (PR #313) live in their own
 * sibling file, useAutoRestoreAnalysis-resilience.test.tsx (qa-intel
 * Monolithic File decomposition, 2026-09-15).
 */
...
describe('useAutoRestoreAnalysis URL-paste auto-restore flow', () => {
  ...
  it('pasting a URL with an existing analysis hydrates the same store contract as history-click restore, setting metadata exactly once', async () => {
    ...
    // The `expect.anything()` second matcher: the hook's fetches now run
    // through fetchWithTimeout, so fetch is called as (url, { signal }) —
    // a single-argument matcher would require the call to have exactly one
    // argument and fail.
    await waitFor(() => {
      expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('/api/analyses/check'), expect.anything());
    });
    await waitFor(() => {
      expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining(`/api/analyses/${ANALYSIS_ID}`), expect.anything());
    });
