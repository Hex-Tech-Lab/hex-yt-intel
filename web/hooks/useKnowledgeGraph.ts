...
export function useKnowledgeGraph(analysisId?: string | null): { graph: KnowledgeGraph; ready: boolean; loading: boolean } {
  ...
  useEffect(() => {
    const isValidWorkerGraph = (g: any): boolean => {
      if (!g || !Array.isArray(g.nodes) || g.nodes.length === 0) return false;
      return g.nodes.every((n: any) => n && typeof n.id === 'string' && typeof n.label === 'string');
    };

    // If we have a high-fidelity knowledge graph generated by the worker, use it directly!
    if (storeKnowledgeGraph && isValidWorkerGraph(storeKnowledgeGraph)) {
      ...
      // Validate or derive rootId dynamically based on out-degree centrality
      let resolvedRootId = storeKnowledgeGraph.rootId;
      const nodeIds = new Set(mappedNodes.map((n: any) => n.id));

      if (!resolvedRootId || !nodeIds.has(resolvedRootId)) {
        console.warn('[useKnowledgeGraph] Missing or invalid rootId in worker graph. Deriving from topology.');
        const outDegrees: Record<string, number> = {};
        mappedNodes.forEach((n: any) => { outDegrees[n.id] = 0; });
        mappedEdges.forEach((e: any) => {
          const count = outDegrees[e.source];
          if (typeof count === 'number') {
            outDegrees[e.source] = count + 1;
          }
        });
        let maxOutDegree = -1;
        let bestRootId = mappedNodes[0]?.id || null;
        for (const id in outDegrees) {
          const val = outDegrees[id];
          if (typeof val === 'number' && val > maxOutDegree) {
            maxOutDegree = val;
            bestRootId = id;
          }
        }
        resolvedRootId = bestRootId;
      }
