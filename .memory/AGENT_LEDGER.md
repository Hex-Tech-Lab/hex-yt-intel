...
---
- [2026-08-03T03:00:00+03:00] [Claude (CC)] [DONE] Investigated 3 bot-flagged review threads on KnowledgeGraphCanvas.tsx (~lines 231/242/252). Verified against actual code + D3 mutation semantics: linkColor/linkWidth still had the pre-fix `data.links.indexOf(l)` stale-index lookup (returns -1 after D3 mutates link objects) and unguarded `l.strength` (NaN for edges with no numeric strength -> Canvas2D silently inherits prior lineWidth) — this worktree's file predates commit f0823c58 (KG edge stroke non-determinism fix, already on branch feat/ui-chat-batch-3 but not this branch). Reapplied that fix: node-ID-based active-edge matching (handles both string and D3-mutated object source/target) + Number.isFinite guard defaulting strength to 0.5. The 3rd flagged spot (nodeCanvasObject isNeighbor check, ~line 297) was a FALSE POSITIVE — already correctly guards `typeof l.source === 'object'` before reading `.id`, safe against D3 mutation, no change needed. qa-intel/contract-auditor: 0 critical/high on diff. type-check clean. vitest: 928 passed, 0 failed. No unit test added (canvas-2D imperative rendering via dynamically-imported react-force-graph-2d, same as prior fix's own precedent — no test added there either). (KnowledgeGraphCanvas.tsx)
