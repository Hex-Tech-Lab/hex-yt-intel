...
- [2026-06-12T00:20:00+03:00] [Antigravity (Agent)] [DONE] Resolved the dynamic Vercel environment callback signature mismatches (401 invalid signature) by passing the frontend origin origin-app-url to the worker. Reverted model cascade back to Claude Haiku 4.5 and Claude Sonnet 4.6 (Nitro). Addressed chat dock state restoration bugs on invalid URL inputs, added clean log levels (logInfo, logOk, logError) to the synthesis.log console, and redirected raw stream deltas to the markdown export cache to prevent LLM apologies from leaking into the log. Staged, committed, and pushed to remote branch feature/1.6.0-prompts-and-limits.
