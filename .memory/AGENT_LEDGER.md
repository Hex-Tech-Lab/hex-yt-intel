...
- [2026-08-07T16:10:00+03:00] [Claude] [DONE] Dependabot #108 js-yaml quadratic CPU (!!omap resolution, GHSA-5p4m-2wfm-xmqj / CVE-2026-59870) fix: transitive via eslint (eslint-config-next devDep chain) and vercel CLI (@vercel/python-analysis chain). Confirmed via `gh api .../dependabot/alerts/108`: first_patched_version for the 4.x line is 4.3.1 (the 5.x fix using a Set instead of O(n^2) indexOf was never backported to 3.x/4.x until 4.3.1). Bumped existing pnpm-workspace.yaml override js-yaml ^4.3.0 -> ^4.3.1 (established override pattern, not a new mechanism). Verified: `pnpm install` succeeded, `pnpm why js-yaml` now resolves js-yaml@4.3.1 (single version, no dedup drift), web tsc --noEmit clean, worker tsc --noEmit -p tsconfig.typecheck.json has pre-existing path-alias failures (confirmed identical via git stash on base commit 9604b3bd, unrelated to this change), qa-intel --ci --compare clean (lockfile/yaml-only diff, no TS files touched). Tangent hunt: paginated `gh api .../dependabot/alerts` for state=open -- alert #108 is the ONLY open alert, nothing else outstanding. PR opened on fix/dependabot-108-js-yaml-omap, not merged (per dispatch instruction).
